...
const BatchList = ({ batches }: { batches: any[] }) => {
  ...
  return (
    <div id="batch-list" class="mt-8 flex flex-col gap-3">
      {batches.map((batch) => (
        <div class="flex items-center p-4 bg-white rounded-xl shadow-sm border border-gray-100 group">
          <div class="flex-grow pl-4 border-l-4 border-blue-500">
            <a
              ...
              class="text-lg font-medium text-blue-600 hover:text-blue-800 transition-colors cursor-pointer block"
            >
              {batch.name}
            </a>
          </div>
...
const PreviewLayout = ({
  ...
  <html lang="en">
    ...
    <body class="bg-gray-50 text-gray-900 font-sans antialiased min-h-screen py-10">
      <div class="max-w-5xl mx-auto px-4">{children}</div>
    </body>
  </html>
);
    ...
    <PreviewLayout title={`${batch.name} - Weekly Schedule`}>
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="bg-blue-600 px-6 py-8 text-white text-center relative">
          <a
            href="/ui/batches"
            class="absolute left-6 top-6 text-blue-200 hover:text-white transition-colors flex items-center gap-1 text-sm font-medium"
          >
            <svg
              class="w-4 h-4"
              fill="none"
        ...
        <div class="p-6 space-y-8">
          {days.map((dayName) => {
            ...
            if (dayEvents.length === 0) {
              return (
                <div class="opacity-50">
                  <h2 class="text-xl font-bold text-gray-900 border-b-2 border-gray-100 pb-2 mb-4">
                    {dayName}
                  </h2>
            ...
            return (
              <div>
                <h2 class="text-xl font-bold text-gray-900 border-b-2 border-gray-200 pb-2 mb-4">
                  {dayName}
                </h2>
                  ...
                  {dayEvents
                    ...
                    .map((ev) => (
                      <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col hover:border-blue-300 transition-colors">
                        ...
                        {ev.description && (
                          <p class="text-sm text-gray-500 mb-3 flex-grow">
                            {ev.description}
                          </p>
                        )}
                        {ev.meetLink && (
                          <a
                            ...
                            class="mt-auto inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-800 transition-colors"
                          >
                            <svg
                              class="w-4 h-4 mr-1"
                              fill="none"
