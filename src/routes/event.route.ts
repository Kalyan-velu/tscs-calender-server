...
events.openapi(createEventRoute,async(c)=>{
  try {
    const {title,description,meetLink,scheduledAt,durationMinutes,batchIds}=c.req.valid('json')
...
events.openapi(assignEventBatchesRequest, async (c) => {
  try {
    ...
    const updatedEvent = await eventRepository.assignToBatches(eventId, batchIds)
